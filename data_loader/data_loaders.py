...
from .data_loaders_gpt2 import MultiModalData_GPT2
from .data_loaders_vip_gpt2 import MultiModalData_VIP_GPT2
import os
import numpy as np
...
class ImageData:
    def __init__(self, args, mode, vocab):
        ...
        self.processed_video_path = self.get_processed_video_path(args['image_cache_path'])
        print('video path : ', self.processed_video_path)
        if not os.path.isfile(self.processed_video_path):
...
#        self.image_dt = load_pickle(self.processed_video_path[mode])
        self.image_dt = load_pickle(self.processed_video_path)

    def get_processed_video_path(self, image_cache_path):
        # For vip
        return Path(image_cache_path+'/processed_video_vip.pickle')
        #return {m: Path(image_cache_path) / ('processed_video_' + m + '.pickle') for m in modes}
...
class DramaQADataLoader(BaseDataLoader):
    def __init__(self, mode, batch_size, shuffle=True, validation_split=0.0, num_workers=1, training=True, vocab=None, **kwargs):
        if kwargs['is_open_ended']:
            if kwargs['is_gpt2']:
                if kwargs['is_vip']:
                    print('is_vip : ', kwargs['is_vip'])
                    dataset = MultiModalData_VIP_GPT2(kwargs, mode=mode)
                else:
                    dataset = MultiModalData_GPT2(kwargs, mode=mode)
            else:
