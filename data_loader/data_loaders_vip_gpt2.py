...
from utils import *
from .preprocess_script_vip_gpt2 import merge_text_data, empty_sub, preprocess_text
...
#        self.vocab_path = '/data/dataset/AnotherMissOh/vocab_gpt2.pickle'
        self.json_data_path = 'input_samples_script.json'
        self.pickle_data_path = 'input_samples_script.pickle'
...
#            self.vocab = build_word_vocabulary(self.args, self.tokenizer, self.json_data_path)
        ...
        # load data
        if not self.args['remove_coreference']:
            print("Loading processed dataset from path: %s." % self.pickle_data_path)
            print('self.pickle_data_path : ', self.pickle_data_path)
            self.data = load_pickle(self.pickle_data_path)
        else:
            print("Loading processed dataset from path: %s." % self.nc_data_path)
            self.data = load_pickle(self.nc_data_path)
...
class MultiModalData_VIP_GPT2(Dataset):
...
#            'str_ans' : change_str
    ...
    def process_image(self, idx, data, max_length = 80):
        text = self.text[idx]
        vid = text['vid']
        vgg_path = '/data/dataset/AnotherMissOh/vggish_v0.6/' + vid +'.npy'
        i3d_flow_path = '/data/dataset/AnotherMissOh/i3d_flow_v0.6/' + vid + '.npy'
        i3d_rgb_path = '/data/dataset/AnotherMissOh/i3d_rgb_v0.6/' + vid + '.npy'
    ...
    def __getitem__(self, idx):
        ...
        input_ids, token_type_ids = data_for_gpt(data, self.tokenizer, self.meta, self.vip)
        bbfts = []
        ...
        que = data['que']
        return input_ids, token_type_ids, que, bbfts
    ...
    def collate_fn(self, batch):
        input_ids_list, token_type_ids_list, que_list, bbfts_list = [], [], [], []
        for data in batch:
            input_ids_list.append(data[0])
            token_type_ids_list.append(data[1])
            que_list.append(data[2])
            bbfts_list.append(data[3])
        ...
        return input_ids, token_type_ids, input_mask, que_list, bbfts_list
