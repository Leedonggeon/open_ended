...
class PredictDataLoader:
    ...
    def __init__(self, args, tokenizer = None, vocab = None, device = None):
        ...
        self.image_dt = load_pickle('/data/dataset/AnotherMissOh/AnotherMissOh_images/cache_v2/processed_video_vip.pickle')
    ...
    def get_data(self, vid, question, answer = None):
        ...
        if answer is None:
            data['vgraphs'] = self.get_meta_data(vid, self.args['visual_type'])
            data['que'] = self.get_question(question)
        else:
            data['que'], data['ans'] = self.get_qa(question, answer)
            data ['bbfts'], data['vgraphs'] = self.get_bbft(vid)
    ...
    def get_subtitle(self, vid, answer):
        ...
        if subtitle != self.empty_sub:
            ...
            for sub in subtitle['contained_subs']:
                sub['et'] = float(sub['et'])
                sub['st'] = float(sub['st'])
                sub['speaker'] = speaker_index[sub['speaker']]
                if answer == None:
    ...
    def load_visual_data(self, vid, visual_data_path = '/data/dataset/AnotherMissOh/AnotherMissOh_Visual.json'):
        visual_datas = read_json(visual_data_path)
        with open(visual_data_path, 'r') as f:
            visual_datas = json.load(f)
        visual_data =[]
        if vid in visual_datas:
            visual_data = visual_datas[vid]
        
        return visual_data
    
    def get_meta_data(self, vid, visual_type = "shot"):
        visual_datas = self.load_visual_data(vid)
        meta_datas = []
        for v in visual_datas:
            ...
            for p in persons:
                ...
                #### Fix Me ####
                person_id_idx = self.none_index if person_id == '' else speaker_index[person_id]
        
                behavior = p['person_info']['behavior'].lower()
                behavior_idx = self.pad_index if behavior == '' else self.tokenizer.convert_tokens_to_ids(behavior.split()[0]) 
        
                emotion = p['person_info']['emotion'].lower()
                emotion_idx = self.pad_index if emotion == '' else self.tokenizer.convert_tokens_to_ids(emotion)
