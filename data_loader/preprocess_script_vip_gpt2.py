...
def merge_text_data(args, json_data_path):
    ...
    new_path = json_data_path
    qa_path =  args['data_path']
...
def preprocess_text(tokenizer, split_tool, json_data_path, save_path):
    ...
    texts = read_json(json_data_path)
    for e in texts:
        ...
        if subtitle != empty_sub:
            ...
            for sub in subtitle['contained_subs']:
                sub['et'] = float(sub['et'])
                sub['st'] = float(sub['st'])
                sub['speaker'] = sub['speaker']#speaker_index[sub['speaker']]  # to speaker index
