...
def data_for_gpt(data, tokenizer, meta, vip = False):
    ...
    for spkr, script in zip(data['spkr'], data['script']):
        if len(sequence) > 500:
            break
        if spkr is 0:
            sequence_spkr = [speaker] + [eos]
        else:
            spkr = [tokenize(spkr, tokenizer)]
            sequence_spkr = [speaker] + spkr[0] + [eos]
        # remove deprecated <sos>
        if len(script) > 0:
            del script[0]
        # for flattening script
        scripts = []
...
#        sequence_script = [subtitle] + scripts + [eos]
        sequence_script = [subtitle] + scripts
...
def sample_sequence(model, input_ids, token_type_ids, tokenizer, device, max_length = 30, temperature = 0.7, top_k = 8, top_p = 0.9, min_length = 1, video=None, bbfts = None):
    ...
    for i in range(max_length):
        ...
        if bbfts is not None:
            ...
            token_ids = torch.cat([torch.ones((bbfts.size(0), bbfts.size(1))).long().cuda()
                                   * tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[6]),
                                   token_ids], dim=1)
        if video is not None:
            input_embs = torch.cat([model.video_ff(video), input_embs], dim=1)
            token_ids = torch.cat([torch.ones((video.size(0), video.size(1))).long().cuda()
